...
compile_error!(
    "Can't use `std` and async runtime simultaneously. Consider adding `default-features = false` to the `ruvchainctl` flags"
);
...
mod tests {
    use super::*;
    const SOCKET_PATH: &str = env!("RUVCHAIN_SOCKET");
